function buildTable(numLeft, numTop, rows) {
    ...
    for (let i = 0; i < rows; i++) {
        ...
        for (let j = 0; j < 10; j++) {
            ...
            td.innerText = `${numLeft + i} x ${numTop + j} = ${product(numLeft + i, numTop + j, false)}`
            tr.append(td)
        }
        tbody.append(tr)
    }
    numLeftInput.onkeydown = numTopInput.onkeydown = rowsInput.onkeydown = (e) => {
        if (e.key == 'Enter') buildTable(+numLeftInput.innerText, +numTopInput.innerText, +rowsInput.innerText)
    }
...
table.onmousemove = e => {
    if (e.target.tagName == 'TD' || e.target.tagName == 'TH') {
        ...
        [...table.rows].forEach(row => row.cells[i].classList.add('selected'))
    }
}
